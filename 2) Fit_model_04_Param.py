...
def model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3, V4, show_more = False):
    global R_rot_original, V_rot_original, R_patch, R_range_min, R_range_max
    V_patch = np.array([V1, V2, V3, V4])
...
def sigma_model(l, b, r, zsun, Usun, Vsun, Wsun, sigma_zsun, sigma_Usun, sigma_Vsun, sigma_Wsun, V1, V2, V3, V4, sigma_V1, sigma_V2, sigma_V3, sigma_V4):
    yr2sec = 3600*24*365
    pc2km = 3.26156*299792.458*yr2sec
    deg2rad = np.pi/180
    radsec2masyr = (3600*1000/deg2rad)*yr2sec
    ...
    pml, pmb = model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3, V4)
    dpml_dzsun = (model(X, zsun+eps_zsun, Usun, Vsun, Wsun, V1, V2, V3, V4)[0]-pml)/eps_zsun
    dpml_dUsun = (model(X, zsun, Usun+eps_Usun, Vsun, Wsun, V1, V2, V3, V4)[0]-pml)/eps_Usun
    dpml_dVsun = (model(X, zsun, Usun, Vsun+eps_Vsun, Wsun, V1, V2, V3, V4)[0]-pml)/eps_Vsun
    dpml_dWsun = (model(X, zsun, Usun, Vsun, Wsun+eps_Wsun, V1, V2, V3, V4)[0]-pml)/eps_Wsun
    dpml_dV1 = (model(X, zsun, Usun, Vsun, Wsun, V1+eps_V, V2, V3, V4)[0]-pml)/eps_V
    dpml_dV2 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2+eps_V, V3, V4)[0]-pml)/eps_V
    dpml_dV3 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3+eps_V, V4)[0]-pml)/eps_V
    dpml_dV4 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3, V4+eps_V)[0]-pml)/eps_V

    dpmb_dzsun = (model(X, zsun+eps_zsun, Usun, Vsun, Wsun, V1, V2, V3, V4)[1]-pmb)/eps_zsun
    dpmb_dUsun = (model(X, zsun, Usun+eps_Usun, Vsun, Wsun, V1, V2, V3, V4)[1]-pmb)/eps_Usun
    dpmb_dVsun = (model(X, zsun, Usun, Vsun+eps_Vsun, Wsun, V1, V2, V3, V4)[1]-pmb)/eps_Vsun
    dpmb_dWsun = (model(X, zsun, Usun, Vsun, Wsun+eps_Wsun, V1, V2, V3, V4)[1]-pmb)/eps_Wsun
    dpmb_dV1 = (model(X, zsun, Usun, Vsun, Wsun, V1+eps_V, V2, V3, V4)[1]-pmb)/eps_V
    dpmb_dV2 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2+eps_V, V3, V4)[1]-pmb)/eps_V
    dpmb_dV3 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3+eps_V, V4)[1]-pmb)/eps_V
    dpmb_dV4 = (model(X, zsun, Usun, Vsun, Wsun, V1, V2, V3, V4+eps_V)[1]-pmb)/eps_V
...
V1_arr, V2_arr, V3_arr, V4_arr = np.zeros((num_rot_curve_params, num_its+1))
V1_arr[0] = V_patch[0]
V2_arr[0] = V_patch[1]
V3_arr[0] = V_patch[2]
V4_arr[0] = V_patch[3]

V1_err_arr, V2_err_arr, V3_err_arr, V4_err_arr = np.zeros((num_rot_curve_params, num_its+1))
V1_err_arr[0] = np.nan
V2_err_arr[0] = np.nan
V3_err_arr[0] = np.nan
V4_err_arr[0] = np.nan
...
for it in tqdm(range(num_its), leave = False, desc = 'Fitting model iteratively'):
    ...
    for i in range(len(df)):
        pml_th, pmb_th = model((l[i], b[i], r[i]), zsun_best, Usun_best, Vsun_best, Wsun_best, V1_best, V2_best, V3_best, V4_best)
        ...
        sig_pml_th, sig_pmb_th = sigma_model(l[i], b[i], r[i],
                                             ...
                                             V1_best, V2_best, V3_best, V4_best,
                                             V1_best_err, V2_best_err, V3_best_err, V4_best_err)
...
# RESULTS
# Insert Rotation Curve Patch
V_patch_fitted = np.array([V1_best, V2_best, V3_best, V4_best])
V_fit_err = np.array([V1_best_err, V2_best_err, V3_best_err, V4_best_err])
...
dV_dR = np.mean([dV_dR_1[-1], dV_dR_2[-1], dV_dR_3[-1]])/(num_rot_curve_params-1)
dV_dR_err = np.sqrt(np.sum(np.array([dV_dR_1_err[-1], dV_dR_2_err[-1], dV_dR_3_err[-1]])**2))/(num_rot_curve_params-1)
R0, zsun = 8178, 20
Rc = np.sqrt((R0**2)-(zsun**2))
Vc = VelGal(Rc, R_rot, V_rot, R_range_min, R_range_max)
rr = (Rc-R_patch[0])/(R_patch[-1]-R_patch[0])
Vc_err = np.sqrt((rr*V4_err_arr[-1])**2+((1-rr)*V1_err_arr[-1])**2)
V0_R0 = (Vc/Rc)*1000
A = 0.5*(V0_R0-dV_dR)
B = -0.5*(V0_R0+dV_dR)
...
for i in range(len(df)):
    rGal, vr, pl, pb = model((l[i], b[i], r[i]), zsun_best, Usun_best, Vsun_best, Wsun_best,
                              V1_best, V2_best, V3_best, V4_best, show_more = True)
...
plt.savefig('Runaway Histogram 04.png', format = 'png', dpi = 400)
plt.show()
...
plt.savefig('Rotation Curve 04.png', format = 'png', dpi = 400)
plt.show()
